...
for (let i = 0; i < content.length; i++) {
  ...
  if (content[i] === '<' && content[i+1] !== '/' && content[i+1] !== ' ' && /[A-Za-z]/.test(content[i+1])) {
    let tag = '';
    let j = i + 1;
    ...
    if (!isSelfClosing && !['input', 'img', 'br', 'hr', 'path', 'circle', 'line', 'rect', 'polyline', 'polygon', 'SignatureCanvas', 'ThemeToggle'].includes(tag)) {
...
console.log('done checking. Stack remaining:', stack);
